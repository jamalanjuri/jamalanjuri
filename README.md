# Hi, I'm Jamal Anjuri 👋

## AI Specialist | Generative AI Evaluator | Prompt Engineer | Filmmaker | Screenwriter | Expert Video Editor | Content Developer

Founder of Tree House Media Kenya, a creative production studio specializing in film, television, documentaries, digital content, and AI-powered creative workflows.
![RLHF](https://img.shields.io/badge/RLHF-Evaluator-blue)
![LLM](https://img.shields.io/badge/LLM-Assessment-success)
![Prompt Engineering](https://img.shields.io/badge/Prompt-Engineer-orange)
![AI Video Evaluation](https://img.shields.io/badge/AI-Video_Evaluation-red)
![Filmmaker](https://img.shields.io/badge/Film-Producer-black)
### What I Do

- Generative AI Evaluation
- RLHF (Reinforcement Learning from Human Feedback) Evaluation
- Large Language Model (LLM) Training & Assessment
- Prompt Engineering & AI Workflow Design
- AI Video Production
- AI Agent Development
- Documentary Production
- Film Production & Directing
- Screenwriting & Script Development
- Video Editing & Post-Production
- Motion Graphics & Visual Storytelling
- Social Media Automation
- Digital Content Strategy
- Creative Project Management
- Training Data Quality Review
- Content Development for TV, Film, and Digital Platforms
## AI & Creative Specializations

- RLHF Evaluation
- LLM Assessment
- Prompt Engineering
- AI Video Evaluation
- AI Agent Development
- Generative AI Workflows
- Screenwriting
- Video Editing
- Documentary Production
## AI Platforms

- Gemini
- Veo
- Vertex AI
- ChatGPT
- Leonardo AI
- Runway
- Midjourney
- Sora
## Featured Projects

### AI Video Evaluation Portfolio
Evaluation methodologies and quality assessment frameworks for AI-generated video content.

### Outlier AI RLHF Portfolio
Examples of RLHF-aligned evaluation, response ranking, prompt assessment, and quality review workflows.

### Marakumi Bet AI Agent
AI-powered analytics, reporting, retention insights, and business intelligence system.

### Tree House Media AI Projects
AI-assisted filmmaking, content creation, automation, and production workflows.

### Screenwriting & Content Development
Feature films, documentaries, television shows, and awareness campaigns developed for Kenyan audiences.

## GitHub Statistics

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=jamalanjuri&show_icons=true)

### Screenwriting & Content Development

I have written and developed scripts, television concepts, documentaries, and educational content designed to inform, entertain, and create social impact.

#### Too Sweet
A feature-length drama focused on diabetes awareness. The story follows a young protagonist living with diabetes while addressing misconceptions, stigma, treatment, and healthy living practices.

#### Perfect Cones
A sequel-driven crime-awareness film exploring various confidence tricks and scams commonly encountered in Nairobi. The project aims to educate audiences on how to identify and avoid fraudsters and conmen.

#### Healthy Mwananchi
A documentary series featuring multiple episodes focused on lifestyle diseases and public health awareness. Each episode examines a specific condition while debunking myths and promoting evidence-based health information.

#### Triple P TV Productions

- **Jaribu 254** – Youth-focused entertainment and lifestyle programming.
- **Events Show** – Coverage of social, corporate, and community events.
- **Politicking** – Political analysis and current affairs programming examining Kenya's political landscape.
- **Motorcar Show** – Automotive reviews, industry insights, and car culture content.

### AI Platforms

- Gemini
- Veo
- Vertex AI
- ChatGPT
- Leonardo AI
- Runway
- Midjourney
- Sora
- Whisk
- Canva AI
- ElevenLabs

### Featured Projects

#### AI Video Evaluation Portfolio
Evaluation methodologies, benchmarking frameworks, prompt assessment, and quality review processes for AI-generated video content.

#### Outlier AI RLHF Portfolio
Experience evaluating AI model responses, prompt quality, reasoning accuracy, factual consistency, instruction following, and human preference alignment for large language models.

#### Marakumi Bet AI Agent
AI-powered betting analytics, reporting, customer retention insights, and business intelligence system.

#### Tree House Media Kenya
Film, documentary, television, commercial production, digital media creation, AI-assisted content development, and creative consulting.

## Connect With Me

📍 Nairobi, Kenya

💼 LinkedIn:
https://www.linkedin.com/in/jamal-anjuri-b72b1138a/

🎥 Portfolio:
https://drive.google.com/drive/folders/1KDjGNALjQzl2X1Lbt8tGAXUOa57BsMnI

💻 GitHub:
https://github.com/jamalanjuri

📧 Email:
ajamal.ghala@gmail.com
